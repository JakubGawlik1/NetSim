...
void Factory::remove_receiver(NodeCollection<Node>& collection, ElementID id) {
    ...
    for (auto& ramp: cont_r) {
        auto& _preferences = ramp.get_receiver_preferences().get_preferences();
    ...
    for (auto& worker: cont_w) {
        auto& _preferences = worker.get_receiver_preferences().get_preferences();
        for (auto _preference: _preferences) {
            if (_preference.first == receiver_ptr) {
